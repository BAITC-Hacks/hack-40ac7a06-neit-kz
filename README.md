...
|---|---|---|
| Вход: город, дата, тип мероприятия, категория, бюджет; опционально длительность и язык | Форма на главной + `POST /api/match`. Бюджет необязателен: без него фильтр по цене не применяется, а в ответе появляется оговорка | [`app/api/match/route.ts`](app/api/match/route.ts) |
...
- Мини-чат на входе (разбор свободного запроса) не успел войти в версию: запрос собирается формой,
  свободные пожелания вводятся отдельным полем.
