...
| Повторный запуск — тот же порядок | `npm run verify` сверяет 12 сценариев с эталонами (12/12 PASS без ключа) + `npm test` — 21 тест, включая три прогона подряд на идентичность | [`samples/golden/`](samples/golden/), [`tests/core.test.ts`](tests/core.test.ts) |
...
npm test           # 21 тест: детерминизм, инварианты, ловушки данных, стоимость проезда
...
**Откуда цифры.** Снимок цен Aviasales (Travelpayouts Data API, `month-matrix`) на маршрут
Алматы ↔ Астана, `data/flights.json`. Считается сумма двух разных плеч — туда и обратно,
умноженная на состав. Если цены на нужный день в снимке нет, берётся ближайший день в пределах
трёх суток, а затем медиана месяца; какой из вариантов сработал, видно в карточке.

**Почему снимок, а не живой вызов.** Ядро обязано быть чистой функцией без сети: от этого зависят
детерминизм и обещание «проверить без ключа одной командой». Демо на площадке не должно зависеть
от вайфая. В полноценной реализации это живой вызов с кэшем на несколько часов — меняется только
тело `estimateTravelCost`, интерфейс функции остаётся прежним. В рамках хакатона на это сознательно
не пошли из-за времени.

**`npm run verify` и `npm run dev` работают без токена Travelpayouts** — он нужен только для
обновления снимка, то есть нам, а не проверяющему.
