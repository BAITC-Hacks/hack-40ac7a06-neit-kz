...
Параметры `city` и `category` передают то, что площадка уже знает о клиенте, — он попадает
в диалог с заполненным контекстом. На бэкенде площадки менять ничего не нужно.
