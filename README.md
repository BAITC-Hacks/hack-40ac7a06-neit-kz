...
- в поставленных данных **занято 50 дней из 100** у каждого профиля; в декабре — 75% дней;
- при переборе 816 комбинаций «город × категория × формат × дата» три и более подходящих подрядчика
  находятся только в **25% случаев**. В остальных 75% трёх карточек просто не существует:
  38% дают одного-двух, 31% — ноль свободных, 6% — такой категории нет в городе.
...
- ⚠️ **Важная деталь.** В присланном расширенном файле календари исходных 66 профилей оказались
  ...
  а из расширенного файла взяты только 175 профилей, которых в поставке не было.
