...
| Повторный запуск — тот же порядок | `npm run verify` сверяет 10 сценариев с эталонами (10/10 PASS без ключа) + `npm test` — 11 тестов, включая три прогона подряд на идентичность | [`samples/golden/`](samples/golden/), [`tests/core.test.ts`](tests/core.test.ts) |
...
npm test           # 11 тестов: детерминизм, инварианты, ловушки данных
